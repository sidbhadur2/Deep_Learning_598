...
# Parameter to change for different dropout rates
keep_prob_rate = .55
...
print ("Dropout probability: %f" % keep_prob_rate)
...
#W9 = tf.Variable(tf.truncated_normal([3,3,num_channels3,num_channels3], stddev=1.0/np.sqrt(3*3*num_channels3) ))
#W10 = tf.Variable(tf.truncated_normal([3,3,num_channels3,num_channels3], stddev=1.0/np.sqrt(3*3*num_channels3) ))
...
#C9 = tf.nn.relu(tf.nn.conv2d(C8, W9, strides = [1,1,1,1], padding = "SAME") )
#C10 = tf.nn.relu(tf.nn.conv2d(C9, W10, strides = [1,1,1,1], padding = "SAME") )
...
for epochs in range(training_epochs):
    ...
    for i in range(num_batches):         
        ...
        _, cost, accuracy_np = session.run( [train_op, cross_entropy_loss, accuracy], feed_dict={x: batch_x, y: batch_y, keep_prob: keep_prob_rate})
        #test accuracy
    time2 = time.time()      
